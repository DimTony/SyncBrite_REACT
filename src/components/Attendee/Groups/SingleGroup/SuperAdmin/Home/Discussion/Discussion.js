...
function Discussion({ user, group }) {
        ...
        const fetchSuperAdminData = async () => {
          ...
          try {
            ...
            const response = await axios.get(
              `http://localhost:8080/api/users/${group.superAdmin}`,
              {
            ...
            if (response.status === 200) {
              const superAdmin = response.data.user;
              setSuperAdmin(superAdmin);
            } else {
              generateError("FAILED TO FETCH SUPER ADMIN.\n REDIRECTING...");
              console.error("FAILED TO FETCH SUPER ADMIN");
              setTimeout(() => {
                navigate("/attendee/groups");
              }, 3000);
            }
          } catch (error) {
